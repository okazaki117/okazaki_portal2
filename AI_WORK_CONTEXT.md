...
| 担当AI | 役割 | 具体的な作業内容 |
|--------|------|-----------------|
| **Claude Code** | 設計・レビュー | 設計整理、仕様の言語化、全体レビュー、ドキュメント整備、複数ファイルにまたがる変更方針の提示、影響範囲・リスクの説明 |
| **Codex** | 実装・修正 | 仕様が明確な実装・修正、差分ベースのコード変更、小〜中規模の局所リファクタ |

### 標準作業フロー

```
1. 設計フェーズ（Claude）  → 要求の整理、仕様の明確化、変更方針の提示
2. 承認（ユーザー）        → 方針の確認と実装の指示
3. 実装フェーズ（Codex）   → 承認された仕様に基づくコード変更
4. 確認（ユーザー）        → 結果の確認、必要に応じてフィードバック
```

すべてのフェーズ間でユーザーの確認・指示を挟む。AIがフェーズを飛ばして自走しないこと。

### 協業ルール

- **曖昧な要求や大きな設計変更は、必ずClaudeフェーズ（設計整理）を先に挟む**。Codexが曖昧な要求を受けた場合は、実装に入らずユーザーに確認を求める
- **実装作業はCodexが担当することを前提とする**。Claudeはコード変更の方針を示すが、大量のコード変更は行わない
