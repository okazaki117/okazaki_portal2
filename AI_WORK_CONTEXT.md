...
> **このファイルは、複数のAI（Claude Code / OpenAI Codex / Antigravity）が作業する際の共通コンテキストです。**
> 作業開始時に必ず最初に読んでください。
>
...
| **Antigravity** | 一気通貫の実装 | 調査、設計提案、ユーザー承認後の直接実装、検証、ドキュメント保守まで単独で完結可能 |
...
### AIの git 操作に関するルール（ブランチ運用）

以前は `work/ai` ブランチでの作業を必須としていましたが、個人開発であるため**基本的に `main` ブランチへの直接 push を許可**します。
ただし、以下のAIごとの特性に応じた運用を行います。
...
4. 既存仕様を変えない（見た目のみ）
5. commit メッセージが以下の形式であること: `fix(ui): 変更内容の要約（英語）`
条件を満たさない場合は「提案のみ」に留めます。

#### Codex の場合
Codex は人間の明示的な依頼がある場合のみ git 操作（commit / push）を行います。
...
### 標準作業フロー（Antigravity使用時）

```
1. 設計・提案フェーズ         → 要求の整理、仕様の明確化、変更方針の提示
2. 承認（人間）               → 方針の確認と実装の指示
3. 実装フェーズ               → 直接コード変更
4. 確認・検証（人間）         → ブラウザ等での動作確認と最終チェック（※AI自身のブラウザによる自動検証は不要）
```

### 標準作業フロー（Claude / Codex 連携時）
...
- **曖昧な要求や大きな設計変更は、必ずClaudeフェーズ（調査・設計整理）等を先に挟む**。
- **実装作業はAntigravityまたはCodexが行う**。Claudeは許可条件を満たす軽微な修正以外のコード変更は行わない。
...
- `main` ブランチへの直接 commit / push（Antigravity、または許可条件を満たしたClaude、および明示的依頼を受けたCodexのみ）
