...
| 日付 | 内容 |
|------|------|
| 2026/03/01 | **v2.5.4: 自動GETリクエストによるデータ二重登録バグの修正（GAS側での重複チェック・JS側での重複排除を追加）** |
| 2026/02/22 | **v2.5.3: Antigravity専用環境への移行と引継ぎ資料の改訂、UIの微調整（角丸・影の調整）** |
| 2026/02/21 | **v2.5.2: サブスクリストにて課金周期ごとの月額換算表示を追加、AI共同開発ルールの更新** |
| 2026/02/09 | **v2.3.2: GAS動的カラムマッピング導入・DEBUGフラグ自動判定・ピン留めメモ改行表示修正** |
| 2026/02/09 | **v2.3.2: 開発・運用マニュアル作成（MANUAL.html）** |
| 2026/02/09 | **v2.3.1: トップ画像UIの整理（変更・削除ボタン除去、削除は設定画面に移動）** |
| 2026/02/09 | **v2.3.0: 買い物リストUX改善（一括追加・カテゴリ分類）** |
| 2026/02/09 | **v2.2.5: ヘッダーとコンテンツの余白追加（8px）** |
| 2026/02/09 | **v2.2.4: ヘッダー重なり解消（padding-top調整・ヘッダー幅制限追加）** |
